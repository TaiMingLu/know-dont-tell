...
parser.add_argument('--num_doc', type=int, default=50, 
...
if args.all_indices:
    args.indices = [i for i in range(args.num_doc)]
else:
...
print(f"Num Documentation per data: {args.num_doc}")
...
for index in args.indices:
    assert index >= 0 and index < args.num_doc, 'Index should in valid values'

data = get_multi_doc_data(args.data_path, args.num_data, args.num_doc, args.num_variation) 
...
os.makedirs(os.path.join(args.save_path, model_name, str(args.num_doc)), exist_ok=True)
...
for i in tqdm(range(len(data)), desc="Documents", position=0):
    ...
    for index in args.indices:
        ...
        if len(results) >= (args.num_data * args.num_variation * len(args.indices) / 10):
            torch.save(results, os.path.join(args.save_path, model_name,str(args.num_doc), f'subset_{args.num_data * args.num_variation}_{save_index}.pth'))
            save_index += 1
            results = []
